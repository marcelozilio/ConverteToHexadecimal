   ...
   INCLUDE('ABWINDOW.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('CONVE001.INC'),ONCE        !Local module procedure declarations
                       INCLUDE('CONVE002.INC'),ONCE        !Req'd for module callout resolution
                     END


Main PROCEDURE                                             ! Generated from procedure template - Frame

AppFrame             APPLICATION('Geração de Carga Balança URANO'),AT(,,505,318),FONT('Segoe UI Symbol',12,COLOR:Black,FONT:regular,CHARSET:ANSI),CENTER,ICON('WAFRAME.ICO'),STATUS(-1,80,120,45),SYSTEM,MAX,RESIZE,IMM
                       MENUBAR,USE(?Menubar)
                         MENU('Carga Balança'),USE(?FileMenu)
                           ITEM('Gera Arquivo'),USE(?FileGeraArquivo)
                         END
                       END
                     END
